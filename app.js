...
function bindElements() {
  Object.assign(elements, {
    ...
    popupReport: document.querySelector("#popup-report"),
    reportOverlay: document.querySelector("#report-overlay"),
    reportTargetTitle: document.querySelector("#report-target-title"),
    closeReport: document.querySelector("#close-report"),
    reportForm: document.querySelector("#report-form"),
    reportFieldOptionsEl: document.querySelector("#report-field-options"),
    reportDescriptionInput: document.querySelector("#report-description-input"),
    reportNameInput: document.querySelector("#report-name-input"),
    reportFeedback: document.querySelector("#report-feedback"),
    reportSubmitButton: document.querySelector("#report-submit-button"),
...
function renderReportFieldOptions() {
  elements.reportFieldOptionsEl.innerHTML = reportFieldOptions
    .map(
      (option) =>
        `<button class="${state.reportFields.includes(option) ? "choice-pill is-active" : "choice-pill"}" data-field="${option}" type="button">${option}</button>`
    )
    .join("");

  elements.reportFieldOptionsEl.querySelectorAll("button").forEach((button) => {
    button.addEventListener("click", () => {
...
function setReportFeedback(message, isError = false) {
  ...
  elements.reportFeedback.classList.remove("hidden");
  elements.reportFeedback.style.background = isError ? "rgba(217, 111, 75, 0.12)" : "rgba(31, 93, 84, 0.10)";
  elements.reportFeedback.style.color = isError ? "#a24d32" : "#194b44";
}

function clearReportFeedback() {
  elements.reportFeedback.classList.add("hidden");
...
function openReportModal(memory) {
  ...
  state.reportFields = [];
  elements.reportTargetTitle.textContent = `你正在回報：《${memory.title}》`;
  elements.reportForm.reset();
  clearReportFeedback();
  renderReportFieldOptions();
  elements.reportOverlay.classList.remove("hidden");
}

function closeReportModal() {
  elements.reportOverlay.classList.add("hidden");
...
async function submitReport(event) {
  ...
  if (!state.supabase) {
    setReportFeedback("你還沒把 Supabase 金鑰填進 app.js，所以目前不能送出回報。", true);
    return;
  }

  const description = elements.reportDescriptionInput.value.trim();
  ...
  try {
    ...
    elements.reportSubmitButton.disabled = true;
    elements.reportSubmitButton.textContent = "送出中…";
    clearReportFeedback();
    ...
    const payload = {
      ...
      description,
      reporter_name: elements.reportNameInput.value.trim() || null,
      status: "pending"
    };

    const { error } = await state.supabase.from("memory_reports").insert(payload);
    if (error) {
      setReportFeedback(formatSupabaseFailure("回報送出", error), true);
      return;
    }
    ...
    elements.reportSubmitButton.disabled = false;
    elements.reportSubmitButton.textContent = "送出回報";
  }
}
...
function renderPopup() {
  ...
  elements.popupReport.onclick = () => openReportModal(memory);
}
...
function wireEvents() {
  ...
  elements.reportOverlay.addEventListener("click", (event) => {
    if (event.target === elements.reportOverlay) closeReportModal();
  });
